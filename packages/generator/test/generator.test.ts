...
describe("generateSdk", () => {
  ...
  it("emits an ambient tools.d.ts for the code-mode surface", async () => {
    tmpPath = await mkdtemp(join(tmpdir(), "tack-generator-ambient-"));
    const config: TackConfig = {
    ...
    expect(dts).toContain("const tools: {");
    expect(dts).toContain('readonly "grafana": {');
    expect(dts).toContain('readonly "alerting": {');
    expect(dts).toContain(
      '"list"(args?: GrafanaAlertingRulesListInput): Promise<CodeModeResult<GrafanaAlertingRulesListOutput>>;'
    );
    expect(dts).toContain(
    ...
    await writeFile(
      join(tmpPath, "tools-usage.generated.ts"),
      [
        "/* Generated by Tack. Do not edit directly. */",
        "export async function useTools() {",
        "  const r = await tools.grafana.datasources.list();",
        "  if (r.ok) { return r.data; }",
        "  const listed = await tools.grafana.alerting.rules.list({ rule_uid: \"x\" });",
        "  emit(listed);",
  ...
  it("keeps tools.d.ts stable across discovery timestamps", async () => {
    tmpPath = await mkdtemp(join(tmpdir(), "tack-generator-ambient-stable-"));
    const outDirA = join(tmpPath, "a");
    const outDirB = join(tmpPath, "b");
    const config: TackConfig = { servers: { fake: { transport: "stdio", command: "node" } } };
    const discovered = [
      {
        serverId: "fake",
    ...
    await generateSdkPromise({
      manifest: buildManifest(config, discovered, new Date("2026-07-23T00:00:00.000Z")),
      outDir: outDirA
    });
    await generateSdkPromise({
      manifest: buildManifest(config, discovered, new Date("2026-07-24T00:00:00.000Z")),
      outDir: outDirB
    });

    expect(await readFile(join(outDirB, "tools.d.ts"), "utf8"))
  ...
  it("tolerates a namespace that shadows a tools builtin", async () => {
    ...
    const manifest = buildManifest(
      ...
      [
        {
          serverId: "search",
          tools: [{ name: "run", inputSchema: { type: "object", properties: {}, additionalProperties: false } }]
        }
      ],
      new Date("2026-07-23T00:00:00.000Z")
    );
