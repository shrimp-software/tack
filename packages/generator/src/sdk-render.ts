...
function renderIndex(
  ...
  return [
    ...
    "  return {",
    "    invoke: <TStructured = unknown>(toolId: string, args: unknown, options?: Parameters<TackRuntime[\"invoke\"]>[2]): Promise<TackResult<TStructured>> => {",
    "      if (!invoke) {",
    "        return Promise.reject(new Error(\"Tack runtime invoke is required\"));",
    "      }",
    "      return invoke.call(runtime, toolId, args, options) as Promise<TackResult<TStructured>>;",
    "    },",
    "    close: (): Promise<void> => {",
    "      if (!close) {",
