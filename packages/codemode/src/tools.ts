...
const __tackToolPathError = (path) =>
  new Error((path.length === 0 ? "tools" : "tools." + path.join(".")) + " is a lazy proxy and cannot be enumerated. Call tools.search({ query: \\"\\" }) to list namespaces, then tools.search({ query: \\"\\", namespace }) for its operations.");
