...
export function createExecuteDescription(
  ...
  const lines = [
    ...
    "",
    "1. `Object.keys(tools)` lists namespaces; `Object.keys(tools.<ns>)` lists a namespace's paths.",
    "   `tools.search({ namespace: \"<ns>\" })` returns the same with descriptions + `params`",
    "   (required input keys); `tools.search({ query })` keyword-searches across everything.",
    "2. `tools.describe.tool({ path })` for the full input schema when `params` isn't enough",
    "   (`{ types: true }` adds TypeScript defs).",
    "3. Call via `tools.<namespace>.<...>(args)` or `tools.call(path, args)`. Await calls. Don't",
